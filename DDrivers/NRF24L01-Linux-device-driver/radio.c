...
int radio_init(void)
{
	...
	GpioRegisters_s = (struct GpioRegisters *)ioremap(GPIO_BASE, sizeof(struct GpioRegisters));
...
void nrf_interrupt_clear(void)
{
	uint8_t *val;
	uint8_t temp;

	val = nrf_xfer(STATUS,1,NULL,R);
	temp = val[0];

	temp |= (1<<RX_DR);

	nrf_xfer(STATUS,1,&temp,W);

}


void nrf_CRCO_enable(void)
{
	uint8_t *val;
	uint8_t temp;
